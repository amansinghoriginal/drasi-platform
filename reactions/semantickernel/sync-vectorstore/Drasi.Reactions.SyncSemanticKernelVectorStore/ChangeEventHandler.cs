...
public class ChangeEventHandler : IChangeEventHandler<QueryConfig>
{
    ...
    public async Task HandleChange(ChangeEvent evt, QueryConfig? queryConfig)
    {
        ...
        try
        {
            ...
            // Note: We no longer pass the collection since sync points are in metadata collection
        ...
        QueryConfig queryConfig)
    {
        ...
        // Filter out any sync point metadata documents (defensive check)
        // These should never come through as regular data, but we check to be safe
        var filteredUpserts = allUpserts.Where(result =>
        {
            if (result.TryGetValue(queryConfig.KeyField!, out var keyValue))
            {
                var key = keyValue?.ToString() ?? "";
                // Skip if this is a sync point key (format: sync_{queryId})
                if (key.StartsWith("sync_"))
                {
                    _logger.LogWarning("Filtered out sync point key from regular processing: {Key}", key);
                    return false;
                }
            }
            return true;
        ...
        if (filteredUpserts.Count == 0)
        {
            _logger.LogDebug("No documents to upsert for query {QueryId}", evt.QueryId);
            return;
        }

        _logger.LogDebug("Processing {Count} documents for upsert in query {QueryId}", filteredUpserts.Count, evt.QueryId);
        ...
        try
        {
            ...
            else
            {
                _logger.LogWarning("No valid vector documents generated from {Count} query results for query {QueryId}", 
                    filteredUpserts.Count, evt.QueryId);
            }
        }
        catch (Exception ex)
        {
        ...
        try
        {
            foreach (var deletedResult in evt.DeletedResults)
            {
                try
                {
                    ...
                    if (key.StartsWith("sync_"))
                    {
                        _logger.LogWarning("Filtered out sync point key from deletion: {Key}", key);
                        continue;
                    }
