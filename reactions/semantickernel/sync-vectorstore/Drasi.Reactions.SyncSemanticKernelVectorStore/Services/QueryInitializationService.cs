...
public class QueryInitializationService : IQueryInitializationService
{
    ...
    private async Task TestVectorStoreConnectivity(CancellationToken cancellationToken)
    {
        ...
        try
        {
            ...
            // Note: We're not deleting the test collection as not all vector stores support immediate deletion
            // The test collection name includes a GUID so it won't conflict with anything
