...
async function verifyEmbeddingCreation(reactionName, expectedCount) {
  try {
    ...
    const logs = cp.execSync(
      `kubectl logs -n drasi-system deployment/${deploymentName} -c reaction --tail=200`,
      { encoding: 'utf8' }
    );
...
beforeAll(async () => {
  ...
  // Replace Azure OpenAI key placeholder if environment variable exists
  const azureKey = process.env.AZURE_OPENAI_KEY;
  if (!azureKey) {
    console.warn("WARNING: AZURE_OPENAI_KEY environment variable not set!");
    console.warn("The test will likely fail without a valid Azure OpenAI API key.");
    console.warn("Set it with: export AZURE_OPENAI_KEY='your-api-key'");
    // For CI/CD, you might want to throw an error here instead:
    // throw new Error("AZURE_OPENAI_KEY environment variable is required");
  } else {
    console.log("Azure OpenAI key found, configuring reactions...");
  ...
  dbPortForward = new PortForward("sk-vectorstore-postgres", 5432, "default");
  ...
  signalrFixture = new SignalrFixture(["sk-products-query", "sk-products-with-category-query"]);
...
describe("InMemory Vector Store E2E Tests", () => {
  ...
  test("Initial state sync - Join query", async () => {
    ...
    const queryData = await signalrFixture.requestReload("sk-products-with-category-query");
    ...
    const embeddingsCreated = await verifyEmbeddingCreation("sk-inmemory-join-reaction", 5);
    expect(embeddingsCreated).toBe(true);
  ...
  test("Insert operations - Single row", async () => {
    ...
    const insertPromise = signalrFixture.waitForChange("sk-products-query",
  ...
  test("Insert operations - Multiple rows", async () => {
    ...
    await waitFor({
      actionFn: async () => {
        const data = await signalrFixture.requestReload("sk-products-query");
  ...
  test("Update operations - Simple update", async () => {
    ...
    const updatePromise = signalrFixture.waitForChange("sk-products-query",
      change => change.op === "u" && 
  ...
  test("Update operations - Join query cascade", async () => {
    ...
    const updatePromise = signalrFixture.waitForChange("sk-products-with-category-query",
      change => change.op === "u" && 
  ...
  test("Delete operations - Single row", async () => {
    ...
    const deletePromise = signalrFixture.waitForChange("sk-products-query",
  ...
  test("Delete operations - Cascade delete in join", async () => {
    ...
    const deletePromise = signalrFixture.waitForChange("sk-products-with-category-query",
    ...
    const simpleQueryData = await signalrFixture.requestReload("sk-products-query");
